...
export const channelsApi = {
  ...
  getById: (id: number | string) => api.get<Channel>(`channels/${id}`),
  getByName: (name: string, userId: number | string) =>
    api.get<Channel>(`channels/name/${encodeURIComponent(name)}/user/${userId}`),
  getMyChannel: () => api.get<Channel>('channels/my'),
  getStats: (id: number | string) => api.get(`channels/${id}/stats`),
  ...
  update: (id: number | string, data: Partial<Channel> | FormData) => {
    if (data instanceof FormData) return api.upload(`channels/${id}`, data, 'PUT');
    return api.put(`channels/${id}`, data);
  },
  delete: (id: number | string) => api.delete(`channels/${id}`),
  subscribe: (channelId: number | string, userId: number | string) =>
    api.post(`channels/${channelId}/subscribe`, { userID: userId }),
  unsubscribe: (channelId: number | string, userId: number | string) =>
